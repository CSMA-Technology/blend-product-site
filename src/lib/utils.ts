...
export const transformPlaylistForClient = (playlist: Database.Playlist): (string | null)[][] =>
  playlist.words?.map((word) => word.map((letter) => (letter === false ? null : letter))) ?? [];

export const transformPlaylistForDatabase = (playlist: Database.Playlist): (string | false)[][] =>
