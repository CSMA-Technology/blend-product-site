...
if (dev) {
    if (FIREBASE_AUTH_EMULATOR_HOST && FIREBASE_DATABASE_EMULATOR_HOST) {
        ...
        console.warn(`Dev environment detected, but at least one of FIREBASE_AUTH_EMULATOR_HOST and FIREBASE_DATABASE_EMULATOR_HOST is not set. Firebase Admin SDK will connect to the configured live instance: ${PUBLIC_DEPLOY_CONTEXT}`);
    }
}
