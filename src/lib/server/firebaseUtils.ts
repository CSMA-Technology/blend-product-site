...
type CheckSessionAuthOptions = { loginRedirect?: string, authFunction?: (decodedToken: DecodedIdToken) => Promise<boolean> | boolean }
/**
...
export const checkSessionAuth = async (cookies: Cookies, options: CheckSessionAuthOptions = {}) => {
    ...
    if (!authFunction || (await authFunction(decodedToken)))
        return decodedToken;
