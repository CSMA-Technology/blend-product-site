...
import { getStripeCustomerWithSubscriptions, isOrganizationMember, isStripeCustomerSubscribedToBlendPro } from '$lib/server/subscriptionUtils';
...
export const POST = (async (event) => {
  ...
  if (!isStripeCustomerSubscribedToBlendPro(sourceCustomer) && !isOrganizationMember(sourceUid)) throw error(401);
  if (!isStripeCustomerSubscribedToBlendPro(targetCustomer) && !isOrganizationMember(targetUid))
